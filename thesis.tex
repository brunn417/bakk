...
Die Zielsetzung dieser Arbeit ist die Technologieevaluierung des aktuellen Stand der Technik und die Implementierung einer den Anforderungen entsprechende Web-Schnittstelle.
Um diese Aufgabenstellung dem wissenschaftlichen Standard gerecht umzusetzen, ist die angewandte Methodik dieser Arbeit eine Kombination aus theoretischen, praktischen und empirischen Analysen.

In \nameref{sec:stateOfTheArt} wurde bereits der erste Schritt der theoretischen Analyse durchgeführt. 
Dabei wurde eine Literaturrecherche und Analyse vorhandener Forschung und Stand der Technik Technologien im Bereich Web-Schnittstellen gemacht.
Im nächsten Schritt erfolgt eine Anforderungsanalyse in Zusammenarbeit mit den Stakeholdern. 
Zur Veranschaulichung und detaillierten Darstellung der Anforderungen wird die Unified Modeling Language (UML) eingesetzt, um klare und verständliche Modelle zu erstellen.

Wenn alle funktionalen und nicht-funktionalen Anforderungen analysiert sind, wird eine Schnittstellen-Technologie gewählt, und folglich der Entwurf entwickelt.
Danach folgt die prototypische Implementierung mit einem den Anforderungen entsprechendem Hypertext Preprocessor (PHP) Web-Framework.
Zum Abschluss wird die entwickelte Schnittstelle anhand Experteninterviews evaluiert.
Die Ergebnisse der Interviews werden analysiert und darauf stützend die Qualität der API beurteilt.
