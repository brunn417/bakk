...
Im Rahmen der theoretischen Analyse, die in \nameref{sec:stateOfTheArt} eingehend beschrieben wurde, erfolgte zunächst eine umfassende Literaturrecherche sowie die Analyse bestehender Forschungsarbeiten und Technologien im Bereich der Web-Schnittstellen. 
...
\chapter{Anforderungsanalyse und Design}
...
Aus den oben beschriebenen \nameref{subsec:useCases} ergeben sich somit die funktionalen Anforderungen an die Schnittstelle:

\begin{enumerate}[label=\textbullet, leftmargin=*]
	\item \textbf{Benutzerdaten abrufen:}
	\begin{itemize}
		\item Entwicklung eines Endpunkts zur Ermittlung der Benutzerdaten mittels eines case-insensitiven Substrings des Namens.
		\item Rückgabe einer Liste aller Benutzer:innen, die den Suchkriterien entsprechen, inklusive ihrer eindeutigen IDs.
	\end{itemize}
	
	\item \textbf{Projektdaten abrufen:}
	\begin{itemize}
		\item Bereitstellung eines Endpunkts, der anhand einer eindeutigen Benutzer-ID alle zugehörigen Projekte retourniert.
	\end{itemize}
	
	\item \textbf{Gebäudedaten analysieren:}
	\begin{itemize}
		\item Implementierung spezifischer Endpunkte zur Unterstützung detaillierter Auswertungen von Gebäudedaten.
		\item Entwicklung von Endpunkten zur Rückgabe von Informationen zu allen Kriteriensets, einschließlich Feldnamen, Feldbeschreibungen und Feldtypen.
	\end{itemize}
	
	\item \textbf{Datenintegration für Dashboards:}
	\begin{itemize}
		\item Schaffung einer Datenbasis, die die Entwicklung von Dashboards unterstützt und detaillierte, strukturierte Auswertungen ermöglicht.
	\end{itemize}
	
	\item \textbf{Feldwertabfragen über alle Projekte:}
	\begin{itemize}
		\item Einrichtung eines Endpunkts, der mithilfe eines eindeutigen Feldnamens als Parameter, alle Werte eines spezifischen Feldes über alle Projekte hinweg zurückgibt.
	\end{itemize}
	
	\item \textbf{Einzelprojektabfragen:}
	\begin{itemize}
		\item Bereitstellung eines Endpunkts zur Abfrage von Projektdaten anhand einer eindeutigen Projekt-ID.
	\end{itemize}
\end{enumerate}
...
% projekt mit bestimmten status auch als endpunkt
...
Es soll für interne als auch externe Personen möglich sein, anhand eines definierten Authentifizierungsprotkoll
