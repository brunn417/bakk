...
In der folgenden Auflistung werden die einzelnen Einschränkungen wiederholt aufgelistet und kurz beschrieben.

\begin{enumerate}
	\item \textbf{Client-Server}:     Strikte Trennung von Client und Server Anliegen
	\item \textbf{Zustandslos}:       Jede Client-Anfrage beinhaltet alle notwendigen Informationen
	\item \textbf{Cache}:             Alle Server-Antworten mit (non-)cacheable deklarieren
	
	\item \textbf{Uniform Interface} 
		\subitem \textbf{Identification of Resources}: \\ Jede Ressource hat einen eindeutigen Identifikator (URI)
		\subitem \textbf{Manipulation of Resources through Representations}: \\ Veränderungen ausschließlich über Repräsentationen
		\subitem \textbf{Self-descriptive Messages}: \\ Jede Nachricht kann für sich selbst stehen
		\subitem \textbf{HATEOAS}: \\ Dynamische Client-Interaktionen durch Hyperlinks in Serverantworten
