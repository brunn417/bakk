...
Wohingegen Schnittstellen mit drei oder mehr Pfade in Richtung Create, Read, Update and Delete (CRUD) oder Representational State Transfer (REST) tendieren. \cite{Web-Api-Structures}


\section{REST-APIs}

Der Großteil aller Web-APIs werden als REST-Schnittstellen deklariert, obwohl nur ein minimaler Anteil tatsächlich die REST-Prinzipien erfüllt \cite{Neumann:2021:AnalysisOfRest}.
Zwar setzen die meisten dieser Schnittstellen einige der REST-Aspekte um, aber erfüllen beinahe nie den in Roy Fieldings Dissertation im Jahr 2000 definierten Architekturstil REST \cite{Neumann:2021:AnalysisOfRest}.
Eine Softwarearchitektur sind die Komponenten eines Softwaresystems, die voneinander abhängig sind und miteinander kommunizieren müssen, um ein funktionsfähiges Programm am laufen zu lassen \cite{Fielding:2000:REST}.
Der Architekturstil gibt vor, welche architektonische Vorbedingungen erfüllt werden müssen, um ein System dem Stil zuordnen zu können \cite{Fielding:2000:REST}.
Das Paradigma REST beinhaltet sechs solche Constraints.
