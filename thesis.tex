...
Zusätzlich ermöglicht ein weiterer Endpunkt, basierend auf eindeutigen Feldnamen, das Abrufen aller Projekte, die in einem spezifischen Feld einen bestimmten Wert aufweisen. 
Jedes Projekt ist durch eine eindeutige ID gekennzeichnet, die ebenfalls genutzt werden kann, um alle Projektdaten abzurufen.


\subsection{Übersicht der Endpunkte}

\begin{description}
	\item[\textbf{getUsersByNameSubstring(String name)}] \hfill \\
	\textbf{Parameter:} name - Der Teilstring des Namens, nach dem gesucht wird. \\
	\textbf{Rückgabe:} UserIds, Name - IDs und Namen der Benutzer, die dem Suchkriterium entsprechen.
\end{description}

\begin{lstlisting}[
	caption={Get Users by Name Substring}
	]
